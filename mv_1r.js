...
var rxUser = prompt("Nhập tên tài khoản Manvip", "");
var rxPassword = prompt("Nhập mật khẩu", "");

RxRestart(0, rxUser, rxPassword)
setInterval(()=>{
    RxRestart(0, rxUser, rxPassword)
}, 3600000)
...
function RxRestart(mode, user, pass) {
    if (rxTaiXiu != undefined) rxTaiXiu.closeSocket();
    rcode = mode ? localStorage.current_game_config : RxNewUser(user, pass)
    if (undefined != rcode){
        var mv_info_tmp = JSON.parse(rcode);
    ...
    rxTaiXiu = null;
    rxTaiXiu = new TaixiuSocket;
    rxTaiXiu.connectSocket();
}

function RxNewUser(user, pass) {
    pass = md5(pass)
    var urlLogin = "https://manvip.club/portal/api?" + "c\x3d3\x26un\x3d" + user + "\x26pw\x3d" + pass + "\x26pf\x3d" + "web" + "\x26at\x3d" + ""
    var res = RxHttpGet(urlLogin);
    return res;
}

function RxHttpGet(theUrl)
{
