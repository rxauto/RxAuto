...
RxInitRandom()
currentMoney = null;
function RxBetTaiXiuSuccess() {
        RxGetData();
        rxInfo.betValue += rxInfo._betValue;
        if (rxInfo._betSide == 2) {
...
function RxInitRandom(){
    if (rxInfo.betMode==0){
        ...
        for (var i = 0; i <= 9999; ++i){
            rxAutoRandom[i] = parseInt(Math.random() * (+max - +min) + +min)%2+1; 
        }
    }
...
function RxReadCauGop() {
    ...
    ret[1] = z[rxInfo.gopIndex] == "x" ? 1 : z[rxInfo.gopIndex] == "t" ? 2 : null;
    rxInfo.gopIndex++
    return ret;
}
...
function RxParseSession(a) {
    ...
    __ = setInterval(()=>{
        if (rxInfo.isAuto&&_timer==rxInfo.autoSec&& rxInfo.bettingState) RxAuto()
...
function RxParseResult(a) {
    ...
    RxCheckResult(a.Dice1 + a.Dice2 + a.Dice3 > 10 ? 2 : 1, a.Dice1, a.Dice2, a.Dice3);
}

function RxCheckResult(result, dice1, dice2, dice3){
    var x = (rxInfo._betSide == 2 ? "TÀI" : "XỈU") + "-" + rxInfo.betValue
    var t = (result == 2 ? "TÀI" : "XỈU") + " " + (dice1 + dice2 + dice3) + " (" + dice1 + "-" + dice2 + "-" + dice3 +")"
    x = x + " || " + t;
